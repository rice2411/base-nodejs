...
import {
  ...
  GITHUB_SCOPE,
  GOOGLE_SCOPE,
} from "../../../config/passport/scope";
const passport = require("passport");
import oauth2Controller from "../../controller/api/oatuh2";

const router = express.Router();

router.route("/google").get(passport.authenticate("google", GOOGLE_SCOPE));
router
...
router
  .route("/facebook")
  .get(passport.authenticate("facebook", FACEBOOK_SCOPE));
router
  .route("/facebook/callback")
  .get(passport.authenticate("facebook", CALLBACK_URIS));

router.route("/github").get(passport.authenticate("github", GITHUB_SCOPE));
router
